...
class Generator(nn.Module):
    ...
    def _build_generator(self):
        layers = [nn.Dropout(self.dropout_rate), nn.Linear(self.latent_dim, self.layers[0]), get_activation_by_name(self.activation_hidden)]
...
class Discriminator(nn.Module):
    ...
    def _build_discriminator(self):
        layers = [nn.Dropout(self.dropout_rate), nn.Linear(self.n_features, self.layers[0]), get_activation_by_name(self.activation_hidden)]
        for i in range(1, len(self.layers)):
            layers.extend([
                nn.Dropout(self.dropout_rate),
                nn.Linear(self.layers[i-1], self.layers[i]),
    ...
    def fit(self, X, y=None):
        ...
        for n in range(self.epochs):
            ...
            for X_train_ in dataloader:
                X_train_sel = X_train_[0]
                latent_noise = torch.rand(X_train_sel.size(0), self.latent_dim_G, dtype=torch.float32)
                ...
                loss_D_real = nn.BCELoss()(real_output, torch.ones_like(real_output) * 0.9)
        ...
        self.query_model = QueryModel(self.generator, self.discriminator, self.latent_dim_G)
        optimizer_query = optim.Adam(self.query_model.parameters(), lr=self.learning_rate_query)
        scores = []
        ...
        for i in range(X_norm.shape[0]):
            ...
            for i in range(self.epochs_query):
                ...
                loss_recon_gen = torch.mean(torch.mean(torch.abs(query_sample - sample_gen), axis=-1))
                # Reconstruction loss latent space of discrimator
                loss_recon_disc = torch.mean(torch.mean(torch.abs(sample_disc_latent_original - sample_disc_latent), axis=-1))
