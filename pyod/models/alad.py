...
class ALAD(BaseDetector):
    ...
    dec_layers : list, optional (default=[5,10,25])
        List that indicates the number of nodes per hidden layer for the decoder network.
    ...
    learning_rate_disc: float in (0., 1), optional (default=0.001)
        learning rate of training the discrimators
    ...
    lambda_recon_loss: float in (0., 1), optional (default=0.1)
        if ``add_recon_loss= True``, the reconstruction loss gets multiplied by ``lambda_recon_loss`` 
    ...
    def _build_model(self):
        ...
        for i, l_dim in enumerate(self.dec_layers):
            ...
            dec_hl_dict[layer_name] = Dropout(self.dropout_rate)(
                Dense(l_dim, activation=self.activation_hidden_gen)(last_layer))
    ...
    def train_step(self, data  ):
        ...
        def get_losses():
            ...
            #Discriminatorxx
            out_truexx, _ = self.disc_xx({'I1': x_real, 'I2': x_real}, training=True) #self.Dxx(x_real, x_real)
