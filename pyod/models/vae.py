...
from pyod.utils.torch_utility import LinearBlock, get_criterion_by_name
...
def vae_loss(x, x_recon, z_mu, z_logvar, beta=1.0, capacity=0.0):
    ...
    # KL divergence
    kl_loss = torch.mean(-0.5 * torch.sum(1 + z_logvar - z_mu ** 2 - torch.exp(z_logvar), dim=1), dim=0)
...
class VAE(BaseDeepLearningDetector):
    ...
    def __init__(self,contamination=0.1, preprocessing=True,
        ...
        super(VAE, self).__init__(contamination=contamination,
                                  ...
                                  optimizer_name=optimizer_name, loss_func=vae_loss,
    ...
    def build_model(self, feature_size):
        self.model = VAEModel(feature_size,
