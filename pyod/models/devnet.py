...
class DevNet(BaseDetector):
    ...
    a weakly supervised anomaly detection problem in that the anomalies are 
    partially observed only and we have no labeled normal data.
    
    Parameters
    ----------
    network_depth : int, optional (default=2)
        The depth of the neural network, determining the complexity and the 
        number of layers in the model.
    batch_size : int, optional (default=512)
        The size of data batches for training the network, impacting both
        memory usage and training dynamics.
    epochs : int, optional (default=50)
        The total number of epochs for training the network across the entire dataset.
    nb_batch : int, optional (default=20)
        Number of batches per epoch. This parameter is reserved for future use 
        and currently not directly utilized in training.
    known_outliers : int, optional (default=30)
        The number of known outlier instances in the dataset, which can be used
        to better tailor the training process.
    cont_rate : float, optional (default=0.02)
        The rate of contamination in the dataset, indicating the proportion of
        outliers expected during training.
    data_format : int, optional (default=0)
        The format of the input data, with '0' typically indicating a CSV file format.
    random_seed : int, optional (default=42)
        The seed used for random number generation to ensure reproducibility.
    device : torch.device, optional
        The device (CPU or GPU) on which the network will be trained. Defaults
        to GPU if available.
    contamination : float in (0., 0.5), optional (default=0.1)
        The assumed proportion of outliers in the dataset, used to determine
        the threshold for the decision function.

    Attributes
    ----------
    model : object
        The underlying deep neural network model used for anomaly detection.
        This model is trained in the `fit` method.
    decision_scores_ : numpy array of shape (n_samples,)
        The anomaly scores of the training data as computed by the model.
        Higher scores indicate more abnormal behavior. This array is available
        after the detector is fitted.
    threshold_ : float
        The threshold used to classify a data point as an outlier based on
        the contamination rate. This is computed during model fitting.
    labels_ : numpy array of shape (n_samples,)
        Binary labels for the training data, where '0' denotes an inlier and
        '1' an outlier. These labels are determined by applying the `threshold_`
        to the `decision_scores_`.
    """
    def __init__(self,
                 network_depth=2,
                 batch_size=512,
