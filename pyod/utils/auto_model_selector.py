...
class AutoModelSelector:
    ...
    def call_gpt(self, prompt):
        ...
        messages = [
            {"role": "system",
             "content": 'You are a well-trained data scientist specifically good at machine learning.'},
        ...
        if not hasattr(self, 'data_info'):
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    print(
                        f"Attempt {attempt + 1}/{max_retries} failed to parse data_info.")
                    if attempt == max_retries - 1:
                        print(
                            "Max retries reached. Could not parse dataset information.")
                        return None, None
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if selected_model in self.model_list:
                    ...
                    print(
                        f"Attempt {attempt + 1}/{max_retries} failed: Model '{selected_model}' not in the model list.")
            except json.JSONDecodeError:
                print(
                    f"Attempt {attempt + 1}/{max_retries} failed to parse selection response.")
                if attempt == max_retries - 1:
                    print(
                        "Max retries reached. Could not complete model selection.")
                    return None, None
