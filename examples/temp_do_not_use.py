...
def visualize_new(clf_name, X_train, y_train, X_test, y_test, y_train_pred,
    ...
    X_train_outliers, X_train_inliers = get_sample_indices(X_train, y_train)
    X_train_outliers_pred, X_train_inliers_pred = get_sample_indices(
        X_train, y_train_pred)

    X_test_outliers, X_test_inliers = get_sample_indices(X_test, y_test)
    X_test_outliers_pred, X_test_inliers_pred = get_sample_indices(
        X_test, y_test_pred)
    ...
    fig.add_subplot(221)
    plt.scatter(X_train_inliers[:, 0], X_train_inliers[:, 1], label='inliers',
                color='orange')
...
if __name__ == "__main__":
    ...
    # visualize the results
    visualize_new(clf_name, X_train, y_train, X_test, y_test, y_train_pred,
                  y_test_pred, show_figure=True, save_figure=False)
